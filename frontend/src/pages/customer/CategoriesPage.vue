    ...
    <SectionCard eyebrow="Discover Categories" title="What are you craving?" description="Browse popular categories from our restaurants">
      <template #actions>
        <div class="flex flex-wrap gap-3">
          <input v-model="query" class="field-input w-80" type="search" placeholder="Search categories..." @input="updateRoute" />
          <RouterLink to="/restaurants" class="btn-secondary">All Restaurants</RouterLink>
        </div>
      </template>

      <div class="mt-6 flex flex-wrap gap-3">
        <button class="pill px-5 py-2 text-sm transition" :class="!activeCategory ? 'bg-brand-500 text-white shadow-sm' : 'bg-white text-slate-700 hover:bg-orange-50 hover:text-brand-700'" @click="selectCategory('')">
          All Categories
        </button>
        <button
      ...
      <div v-if="filteredCategories.length" class="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        <button
          v-for="category in filteredCategories"
          :key="category.slug"
          class="group overflow-hidden rounded-xl border border-slate-200 bg-white text-left transition hover:-translate-y-1 hover:shadow-lg"
          @click="selectCategory(category.name)"
        >
          <div class="relative h-44 bg-slate-100">
            ...
            <div class="absolute bottom-3 right-3 rounded-full bg-white/90 px-3 py-1 text-xs font-semibold shadow">
      ...
      <div v-else class="mt-12 rounded-xl border border-dashed border-slate-300 bg-slate-50 py-16 text-center">
        <p class="text-2xl">No matching categories</p>
        <p class="mt-3 text-lg font-medium">Try a different keyword</p>
        <p class="text-slate-500">Search by category or restaurant name.</p>
      </div>
    </SectionCard>
...
const filteredCategories = computed(() => {
  ...
  return restaurantStore.categories.filter((cat) => cat.name.toLowerCase().includes(term) || cat.restaurantNames.some((name) => name.toLowerCase().includes(term)));
});

function getCategoryEmoji(name: string): string {
  const lower = name.toLowerCase();
  if (lower.includes('pizza') || lower.includes('italian')) return 'Pizza';
  if (lower.includes('burger') || lower.includes('fast')) return 'Burger';
  if (lower.includes('asian') || lower.includes('thai')) return 'Noodles';
  if (lower.includes('dessert') || lower.includes('sweet')) return 'Dessert';
  if (lower.includes('healthy') || lower.includes('salad')) return 'Healthy';
