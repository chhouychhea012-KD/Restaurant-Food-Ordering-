...
export async function listOrders() {
  if (useBackendApi) return cachedGet<Order[]>('/orders', undefined, 8_000);
  return dbOrders();
}

export async function listOrdersForCustomer(customerId: string) {
  if (useBackendApi) return cachedGet<Order[]>('/orders', { params: { customerId } }, 8_000);
...
export async function listOrdersForRestaurant(restaurantId: string) {
  if (useBackendApi) return cachedGet<Order[]>('/orders', { params: { restaurantId } }, 8_000);
  return dbOrders().filter((order) => order.restaurantId === restaurantId);
}

export async function listRiderOrders(riderName: string) {
  if (useBackendApi) return cachedGet<Order[]>('/orders', { params: { riderName } }, 8_000);
...
export async function createOrder(payload: {
  ...
  if (useBackendApi) { const order = unwrap<Order>(await api.post('/orders', payload)); clearApiCache('/orders'); clearApiCache('/notifications'); clearApiCache('/activity-logs'); clearApiCache('/analytics'); return order; }
...
export async function updateOrderStatus(orderId: string, status: string, label?: string) {
  if (useBackendApi) { const order = unwrap<Order | null>(await api.patch('/orders/' + orderId + '/status', { status, label })); clearApiCache('/orders'); clearApiCache('/notifications'); clearApiCache('/activity-logs'); clearApiCache('/analytics'); return order; }
  const orders = dbOrders();
...
export async function updateOrder(orderId: string, payload: OrderUpdateInput) {
  if (useBackendApi) { const order = unwrap<Order | null>(await api.put('/orders/' + orderId, payload)); clearApiCache('/orders'); clearApiCache('/notifications'); clearApiCache('/activity-logs'); clearApiCache('/analytics'); return order; }
  const orders = dbOrders();
  const previousOrder = orders.find((order) => order.id === orderId) ?? null;
...
export async function approveRefund(orderId: string, reason: string) {
  if (useBackendApi) { const order = unwrap<Order>(await api.patch('/orders/' + orderId + '/refund', { reason })); clearApiCache('/orders'); clearApiCache('/notifications'); clearApiCache('/activity-logs'); clearApiCache('/analytics'); return order; }
  const orders = dbOrders();
  const previousOrder = orders.find((order) => order.id === orderId) ?? null;
...
export async function deleteOrder(orderId: string) {
  if (useBackendApi) { await api.delete('/orders/' + orderId); clearApiCache('/orders'); clearApiCache('/notifications'); clearApiCache('/activity-logs'); clearApiCache('/analytics'); return true; }
