...
export async function listRoles() {
  if (useBackendApi) return cachedGet<RoleDefinition[]>('/roles', undefined, 30_000);
  return sortRoleDefinitions(dbRoles());
}

export async function listRolePermissions() {
  if (useBackendApi) return cachedGet<string[]>('/roles/permissions', undefined, 60_000);
...
export async function createRole(payload: RoleInput) {
  if (useBackendApi) { const role = unwrap<RoleDefinition>(await api.post('/roles', payload)); clearApiCache('/roles'); return role; }
  const roles = dbRoles();
...
export async function updateRole(roleId: string, payload: RoleInput) {
  if (useBackendApi) { const role = unwrap<RoleDefinition | null>(await api.put('/roles/' + roleId, payload)); clearApiCache('/roles'); return role; }
  const roles = dbRoles();
...
export async function deleteRole(roleId: string) {
  if (useBackendApi) { await api.delete('/roles/' + roleId); clearApiCache('/roles'); return true; }
  const roles = dbRoles();
