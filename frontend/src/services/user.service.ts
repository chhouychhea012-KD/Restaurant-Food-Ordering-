...
export async function listUsers() {
  if (useBackendApi) return cachedGet<User[]>('/users', undefined, 20_000);
  return dbUsers();
}

export async function createUser(payload: UserInput) {
  if (useBackendApi) { const user = unwrap<User>(await api.post('/users', payload)); clearApiCache('/users'); return user; }
  const users = dbUsers();
...
export async function updateUser(userId: string, payload: UserInput) {
  if (useBackendApi) { const user = unwrap<User | null>(await api.put('/users/' + userId, payload)); clearApiCache('/users'); return user; }
  const users = dbUsers();
...
export async function deleteUser(userId: string) {
  if (useBackendApi) { await api.delete('/users/' + userId); clearApiCache('/users'); return true; }
