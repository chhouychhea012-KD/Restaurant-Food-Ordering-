          ...
          <div class="border-b border-slate-200/80 bg-[radial-gradient(circle_at_top_left,rgba(249,115,22,0.15),transparent_28%),linear-gradient(180deg,#ffffff,rgba(248,250,252,0.92))] px-4 py-4 sm:px-7 sm:py-5">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                <h3 data-i18n-ignore class="mt-2 text-xl font-bold text-slate-950 sm:text-2xl">{{ t(title) }}</h3>
                <p v-if="description" data-i18n-ignore class="mt-2 max-w-2xl text-sm text-slate-600">{{ t(description) }}</p>
              </div>
              <button data-i18n-ignore class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-500 transition hover:border-slate-300 hover:text-slate-900" type="button" @click="emit('close')">
                {{ t('Close') }}
              </button>
            </div>
          </div>
