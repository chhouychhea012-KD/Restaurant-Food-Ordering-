    ...
    <div class="flex flex-1 flex-col p-4">
      ...
      <div class="mt-4 grid grid-cols-3 gap-2">
        <div class="rounded-lg bg-slate-50 px-3 py-2 text-xs">
          <p class="font-bold text-slate-950">{{ ratingText }}</p>
...
const ratingText = computed(() => Number(props.restaurant.rating || 0).toFixed(1));

const branchSummary = computed(() => {
  const count = Array.isArray(props.restaurant.branches) ? props.restaurant.branches.length : 0;
